"""
简化的联邦学习脚本（无泄漏追踪功能）
保留水印嵌入和水印聚合功能，移除给每个客户端下发专有模型和追踪功能
"""
...
# 配置 logging
args = parser_args()
log_file_name = args.log_file.replace('.logs', '_simple.logs')
logging.basicConfig(
    level=logging.INFO,
...
class FederatedLearningSimple(Experiment):
    ...
    def construct_model(self):
        if self.model_name == 'resnet':
            model = resnet18(num_classes=self.num_classes, in_channels=self.in_channels,
    ...
    def training(self):
        ...
        train_ldr = DataLoader(self.train_set, batch_size=self.batch_size, shuffle=False, num_workers=0,
                               pin_memory=False)
        val_ldr = DataLoader(self.test_set, batch_size=self.batch_size * 2, shuffle=False, num_workers=0,
                             pin_memory=False)
        ...
        for epoch in range(self.epochs):
            ...
            for i, idx in enumerate(tqdm(idxs_users, desc='Progress: %d / %d' % (epoch + 1, self.epochs))):
                ...
                # 统一调用：始终传�?current_epoch/total_epochs
            ...
            if epoch >= 0 and hasattr(self.trainer, 'multi_loss') and (epoch + 1) % 10 == 0:
                try:
                    ...
                    if stats:
                        logging.info(
                            f'轮次{epoch + 1}梯度统计 - GM:{stats.get("prevGM", 0):.6f} GH:{stats.get("prevGH", 0):.6f} Ratio:{stats.get("prevRatio", 1):.6f}')
            ...
            if (epoch + 1) == self.epochs or (epoch + 1) % 1 == 0:
                ...
                # MultiLoss统计信息（仅�?0轮打印）
                if hasattr(self.trainer, 'multi_loss') and (epoch + 1) % 10 == 0:
                    stats = self.trainer.multi_loss.get_stats()
                    logging.info(
                        f"MultiLoss - GM:{stats['prevGM']:.6f} GH:{stats['prevGH']:.6f} Ratio:{stats['prevRatio']:.6f}")
                    ...
                    if select_by_auc:
                        if auc_val > best_val_auc:
                            ...
                            if early_stop_counter >= self.args.patience:
                                logging.info(
                                    f'Early stopping triggered at epoch {epoch + 1}. Best Val AUC: {best_val_auc:.4f}')
                                break
                    else:
                        if acc_val_label_mean > best_val_acc:
                            ...
                            if early_stop_counter >= self.args.patience:
                                logging.info(
                                    f'Early stopping triggered at epoch {epoch + 1}. Best Val ACC: {best_val_acc:.4f}')
                                break
        ...
        logging.info(
            f'历史最佳 | Acc:{self.logs["highest_acc_ever"]:.4f}(AUC:{self.logs["auc_when_highest_acc"]:.4f}) | '
            ...
            for i, client_id in enumerate(idxs_users):
                try:
                    ...
                    for param_name, global_idx in positions:
                        # 检查参数是否存�?
                        if param_name not in local_ws[i] or param_name not in w_avg:
                            continue
                        ...
                        param_flat[global_idx] = local_flat[global_idx]
...
def main(args):
    ...
    fl = FederatedLearningSimple(args)
    ...
    watermark_suffix = f"wm_{args.watermark_mode}" if hasattr(args,
                                                              'watermark_mode') and args.watermark_mode else "wm_basic"

    file_name = '{}_simple_Dp_{}_iid_{}_{}_ep_{}_le_{}_cn_{}_fra_{:.4f}_{:.4f}{}.pkl'.format(
        formatted_now, args.sigma, args.iid, watermark_suffix,
        args.epochs, args.local_ep, args.client_num, args.frac, best_metric_value, enhanced
    )
    torch.save(logs, os.path.join(save_dir, file_name))
    logging.info(f"训练日志已保�? {file_name}")
    logging.info('-------------------------------Finish--------------------------------------')
