...
class RegAblationExperiment(Experiment):
    """
    正则项消融实验：支持 reg1、reg2、reg3 三个正则项的独立控制
        ...
        if len(self.trace_results) > 0:
            ...
            try:
                ...
                trace_log_path = os.path.join(
                    trace_dir,
                    f'trace_results_reg_ablation_{self.model_name}_{self.dataset}_{self.reg_config_str}_{datetime.now().strftime("%Y%m%d%H%M%S")}.json'
                )
                ...
                with open(trace_log_path, 'w', encoding='utf-8') as f:
                    json.dump({
                        ...
                        'reg_config': self.reg_config_str,
