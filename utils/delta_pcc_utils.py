...
def evaluate_delta_pcc(original_model_state, current_model_state, reconstructor, 
    ...
    try:
        # 1. 从原始模型重建自编码器作为基准
        original_reconstructed_autoencoder = reconstructor.reconstruct_autoencoder(original_model_state)
        ...
        # 4. 从当前模型重建自编码器
        current_reconstructed_autoencoder = reconstructor.reconstruct_autoencoder(current_model_state)
